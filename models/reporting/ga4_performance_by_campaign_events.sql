{{ config (
    alias = target.database + '_ga4_performance_by_campaign_events'
)}}
...
{%- set fields = adapter.get_columns_in_relation(ref('ga4_traffic_sources_events'))
                    |map(attribute="name")
...
WITH 
    ...
    (SELECT 
        ...
        {%- endfor %}
    FROM {{ ref('ga4_traffic_sources_events') }}
